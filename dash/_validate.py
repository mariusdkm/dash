...
def validate_multi_return(output_specs, output_values, callback_id):
    ...
    if len(output_values) != len(output_specs):
        raise exceptions.InvalidCallbackReturnValue(
            f"""
            Invalid number of output values for {callback_id}.
            Expected {len(output_specs)}, got {len(output_values)}
            """
        )
            ...
            if len(output_value) != len(output_spec):
                raise exceptions.InvalidCallbackReturnValue(
                    dedent(
                        ...
                        Expected {len(output_value)}, got {len(output_spec)}
                        output spec: {output_spec!r}
